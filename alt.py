...
#     myembed.add_field(name = "feild1", value = "this is line 1")
...
async def on_raw_reaction_add(payload):
    ...
    if payload.emoji.name == "👧" and payload.message_id == 964916679131942913 :
        role = discord.utils.get(guild.roles, name="bitches")
    elif payload.emoji.name == "👦" and payload.message_id == 964916679131942913 :
        role = discord.utils.get(guild.roles, name="gents")
        if role is not None:
...
async def ify(ctx):
    ...
    gender_embed = discord.Embed(title=f"Welcome!").set_author(name="")
    gender_embed.add_field(name="Choose a gender role", value=f"All roles come with their specific perks\
    \nYou can only choose one role \n\n👦- if u man\n 👧 -if u a woman")
    gender_embed.set_footer(text=f" ")



    react_messasge = await ctx.send(embed=gender_embed)
